...
export class Container extends Emitter {
  ...
  async resolve<T>(token: Token<T>, config?: ProviderResolveConfig<T>): Promise<T> {
    ...
    if (provider) {
      ...
      if (last) {
        return last.resolve(config).then(async (value) => {
  ...
  async resolveAll<T>(token: Token<T>, config?: ProviderResolveConfig<T[]>): Promise<T[]> {
    ...
    if (provider) {
      ...
      return Promise.all(providers.map((provider) => {
        return provider.resolve(config).then(async (value) => {
          await this.emit("resolve", {
            container: this,
            provider,
            token,
  ...
  isResolved<T>(token: Token<T>, config?: ProviderResolveConfig<T>): boolean {
    const provider = this.store.getProvider(token);
    ...
    if (this.parent) {
      return this.parent.isResolved(token, config);
    }
  ...
  async dispose<T>(token: Token<T>, config?: ProviderDisposeConfig) {
    ...
    if (provider) {
      ...
      await Promise.all(providers.map((provider) => {
        return provider.dispose(config).then(async () => {
  ...
  isDisposed<T>(token: Token<T>, config?: ProviderDisposeConfig): boolean {
    const provider = this.store.getProvider(token);

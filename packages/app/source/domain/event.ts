...
export const createEvent = (name: string) => {
  ...
  const decorator = createDecorator(name, (config?: EventConfig) => ({
    ...
    onInit: (context) => {
      ...
      const before = config?.before ? (Array.isArray(config.before) ? config.before : [config.before]) : [];
      const after = config?.after ? (Array.isArray(config.after) ? config.after : [config.after]) : [];
